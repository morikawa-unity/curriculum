#!/usr/bin/env python3
"""
RDSデータベースセットアップスクリプト
AWS RDS環境でのデータベース初期化用
"""
...
# テストユーザーデータ
TEST_USER_DATA = {
    'id': '37240af8-9041-701f-8eca-954b33eefe76',  # Cognitoで作成したユーザーのsub
...
def get_connection(include_db: bool = False) -> Optional[pymysql.Connection]:
    ...
    try:
        ...
        logger.info(f"RDS接続成功: {config['host']}:{config['port']}")
        return connection
    except Exception as e:
        logger.error(f"RDS接続エラー: {str(e)}")
        return None
...
def insert_test_user():
    ...
    try:
        with connection.cursor() as cursor:
            # 既存ユーザーの確認
            cursor.execute("SELECT COUNT(*) FROM users WHERE id = %s", (TEST_USER_DATA['id'],))
            if cursor.fetchone()[0] > 0:
                logger.info("テストユーザーは既に存在します")
                return True
            
            # テストユーザーの挿入
            sql = "INSERT INTO users (id, email, username) VALUES (%s, %s, %s)"
            cursor.execute(sql, (
                TEST_USER_DATA['id'],
            ...
            connection.commit()
            logger.info(f"テストユーザー '{TEST_USER_DATA['username']}' を挿入しました")
        return True
    except Exception as e:
        logger.error(f"テストユーザー挿入エラー: {str(e)}")
        return False
    finally:
        connection.close()
...
def main():
    """メイン処理"""
    print("=" * 60)
    print("プログラミング学習アプリ - RDSデータベースセットアップ")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("RDSデータベースセットアップが完了しました！")
    print("\n接続情報:")
    print(f"  ホスト: {RDS_CONFIG['host']}")
    print(f"  ポート: {RDS_CONFIG['port']}")
    print(f"  データベース: {DATABASE_NAME}")
    print(f"  ユーザー名: {RDS_CONFIG['user']}")
    print("=" * 60)
