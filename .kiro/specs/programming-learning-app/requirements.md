...
プログラミング学習用の Web アプリケーションを開発します。このアプリは学習者がプログラミングスキルを向上させるための機能を提供し、AWS Amplify、Cognito、Lambda、RDS（MySQL）、CloudFormation を使用したフルスタック構成で構築されます。フロントエンドは Next.js App Router + TypeScript でクライアントサイドレンダリング（CSR）として実装し、UI コンポーネントには shadcn/ui + TailwindCSS を使用してモダンで一貫性のあるデザインシステムを構築します。バックエンドは Python（FastAPI）で実装し、AWS Amplify の統合 CI/CD パイプラインを活用します。
...
**ユーザーストーリー:** 開発者として、一貫性のあるモダンな UI デザインシステムを使用したい。そうすることで、保守性が高く美しいユーザーインターフェースを効率的に構築できる。

#### 受け入れ基準

1. UI コンポーネントが作成された時、システムは shadcn/ui コンポーネントライブラリを使用する
2. スタイリングが適用された時、システムは TailwindCSS ユーティリティクラスを使用する
3. コンポーネントが表示された時、システムは一貫したデザイントークン（色、タイポグラフィ、間隔等）を適用する
4. ダークモード切り替えが実行された時、システムは適切なテーマ変更を行う
5. レスポンシブデザインが適用された時、システムは全デバイスで最適な表示を提供する
6. アクセシビリティ要件が確認された時、システムは WCAG 2.1 AA レベルの基準を満たす
